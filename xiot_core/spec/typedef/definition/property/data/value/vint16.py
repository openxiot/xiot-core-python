...
class Vint16(DataValue[int]):
    ...
    def less_equals(self, max_value: DataValue) -> bool:
        if not isinstance(max_value, Vint16):
            return False
        return self._value <= max_value._value

    def validate(self, min_value: DataValue, max_value: DataValue) -> bool:
        if not (isinstance(min_value, Vint16) and isinstance(max_value, Vint16)):
            return False
    ...
    def validate_with_step(self, min_value: DataValue, max_value: DataValue, step: DataValue) -> bool:
        if not (isinstance(min_value, Vint16) and isinstance(max_value, Vint16) and isinstance(step, Vint16)):
            return False
        min_v = min_value._value
        max_v = max_value._value
        step_v = step._value
        ...
        print(f"Vint16.validate failed: {self._value} min: {min_v} max: {max_v} step: {step}")
        return False
    ...
    @staticmethod
    def value_of(obj: object) -> Optional['Vint16']:
        if isinstance(obj, int):
            return Vint16(int(obj))
        return None
    ...
    def __eq__(self, other: object) -> bool:
        if self is other:
            return True
        if not isinstance(other, Vint16):
            return False
