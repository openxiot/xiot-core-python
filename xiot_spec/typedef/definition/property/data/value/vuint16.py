...
class Vuint16(DataValue[int]):
    ...
    def less_equals(self, max_value: DataValue) -> bool:
        if not isinstance(max_value, Vuint16):
            return False
        return self._value <= max_value._value

    def validate(self, min_value: DataValue, max_value: DataValue) -> bool:
        if not (isinstance(min_value, Vuint16) and isinstance(max_value, Vuint16)):
            return False
        min_v = min_value._value
        max_v = max_value._value
        if self._value < min_v or self._value > max_v:
            print(f"Vuint16.validate failed: {self._value} min: {min_value} max: {max_value}")
            return False
        return True

    def validate_with_step(self, min_value: DataValue, max_value: DataValue, step: DataValue) -> bool:
        if not (isinstance(min_value, Vuint16) and isinstance(max_value, Vuint16) and isinstance(step, Vuint16)):
            return False
        min_v = min_value._value
        max_v = max_value._value
        step_v = step._value
        ...
        print(f"Vuint16.validate failed: {self._value} min: {min_v} max: {max_v} step: {step}")
        return False
