...
class TestActionDefinitionCodec(unittest.TestCase):
    def test_codec(self) -> None:
        """Test ActionDefinitionCodec"""
        self._decode("/definition/actions/miot")
        self._decode("/definition/actions/xiot")
    ...
    def _decode_file(self, file_path: pathlib.Path) -> None:
        ...
        # 编解码测试
        definition = ActionDefinitionCodec.decode(original_json)
        encoded_json = ActionDefinitionCodec.encode(definition)
